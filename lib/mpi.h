...
int MPI_Init(void*args, void***argv)
{
    return MPI_Init((int*)args, (char***)argv);
}
...
int MPI_Send(float* buff, int totalcount, int unitsize, int dest_coreID, int TAG, int arg) __attribute__((always_inline));
int MPI_Send(float* buff, int totalcount, int unitsize, int dest_coreID, int TAG, int arg)
{
    dest_coreID = (dest_coreID << 6 ) | 0xf0000000;
    int* dest = (int*)dest_coreID;
//    int tmp;
...
int MPI_Send(double* buff, int totalcount, int unitsize, int dest_coreID, int TAG, int arg)
{
    printf("\nMPI_Send for double\n");
}
...
int MPI_Recv(float* buff, int totalcount, int unitsize, int src_coreID, int TAG, int arg, int* stat) __attribute__((always_inline)); 
int MPI_Recv(float* buff, int totalcount, int unitsize, int src_coreID, int TAG, int arg, int* stat)
{
    src_coreID = (src_coreID << 6 ) | 0xf0000000;
    int* src = (int*)src_coreID;
    while(totalcount){
...
int MPI_Recv(double* buff, int totalcount, int unitsize, int src_coreID, int TAG, int arg, int* stat)
{
    printf("\nMPI_Recv for double\n");
}
...
int MPI_Bcast(int* buf, int totalcount, int unitsize, int root, int arg) 
{
    ...
    //                    "       cmp    %[totalcount], #0\n"
    //                    "       bne     2b"
                        :[buff]"+r"(buf), [totalcount]"+r"(totalcount)
                        :[src]"m"(*src)//[tmp]"r"(tmp)//, "r"(totalcount)
                        :"memory", "cc"
            );
        }
...
int MPI_Bcast(float* buf, int totalcount, int unitsize, int root, int arg) __attribute__((always_inline)); 
int MPI_Bcast(float* buf, int totalcount, int unitsize, int root, int arg) 
{    
	if (myRank == root) 
	{
		// If we are the root process, send our data to everyone
	...
	{
		// If we are a receiver process, receive the data from the root
//		MPI_Recv(buf, totalcount, unitsize, root, 0, arg, 0);
...
int MPI_Scatter(float* send_buf, int send_totalcount, int send_unitsize,
            float* recv_buf, int recv_totalcount, int recv_unitsize,
            int root, int COMM)
{
        int world_rank;
        MPI_Comm_rank(COMM, &world_rank);
...
int MPI_Gather(float* send_buf, int send_totalcount, int send_unitsize,
           float* recv_buf, int recv_totalcount, int recv_unitsize,
           int root, int COMM)
{
        int world_rank;
...
int MPI_Reduce(float *send_buf, float *recv_buf, int total_count, MPI_Datatype datatype, MPI_Op op, int root, int COMM)
{
    //printf("\nMPI_Reduce(float) on core %d is called\n", myRank);
	int temp_count = total_count;
	...
	if (myRank == root)
	{
		// If I am the root process, receive data from everyone else
		int i;
		float temp_buf[default_num_cores];
		while(temp_count > 0)
		{
			temp_buf[root] = *(send_buf+total_count-temp_count);
			for (i = 0; i < default_num_cores; i++)
...
int MPI_Reduce(double *send_buf, double *recv_buf, int total_count, MPI_Datatype datatype, MPI_Op op, int root, int COMM)
{
	int world_rank;
	MPI_Comm_rank(COMM, &world_rank);
	int world_size;
...
int MPI_Allreduce(float *send_buf, float *recv_buf, int total_count, MPI_Datatype datatype, MPI_Op op, int COMM)
{
	int world_rank;
	MPI_Comm_rank(COMM, &world_rank);
	int world_size;
...
int MPI_Allreduce(double *send_buf, double *recv_buf, int total_count, MPI_Datatype datatype, MPI_Op op, int COMM)
{
	int world_rank;
	MPI_Comm_rank(COMM, &world_rank);
	int world_size;
...
int MPI_Alltoall(float *send_buf, int sendcount, MPI_Datatype sendtype, float *recv_buf, int recvcount, MPI_Datatype recvtype, int COMM)
{
	int world_rank;
	MPI_Comm_rank(COMM, &world_rank);
	int world_size;
...
MPI_Barrier(COMM);
	for (i = 0; i < world_size; i++)
	{
		if (i != world_rank)
		{
			MPI_Send(send_buf+i*sendcount, sendcount, sendtype, i, 0, 0);
		}
	}
	int temp = 0;
	while (recvcount-temp>0)
	...
	for(i=0; i<world_size; ++i)
	{
		...
		{
			//Receive the data from the root
			MPI_Recv(recv_buf+i*recvcount, recvcount, recvtype, i, 0, 0, 0);
		}
	}
MPI_Barrier(COMM);
	return 0;
...
int MPI_Alltoall(double *send_buf, int sendcount, MPI_Datatype sendtype, double *recv_buf, int recvcount, MPI_Datatype recvtype, int COMM)
{
	int world_rank;
	MPI_Comm_rank(COMM, &world_rank);
	int world_size;
...
int MPI_Alltoallv(float *send_buf, int*sendcount, int*send_displ, MPI_Datatype sendtype, float *recv_buf, int*recvcount, int*recv_displ, MPI_Datatype recvtype, int COMM)
{
	int world_rank;
	MPI_Comm_rank(COMM, &world_rank);
	int world_size;
...
MPI_Barrier(COMM);
	for (i = 0; i < world_size; i++)
	{
		if (i != world_rank)
		{
			MPI_Send(send_buf+send_displ[i], sendcount[i], sendtype, i, 0, 0);
		}
	}
	int temp = 0;
	while (recvcount[world_rank]-temp>0)
	...
	for(i=0; i<world_size; ++i)
	{
		...
		{
			//Receive the data from the root
			MPI_Recv(recv_buf+recv_displ[i], recvcount[i], recvtype, i, 0, 0, 0);
		}
	}
	return 0;
}

int MPI_Alltoallv(double *send_buf, int*sendcount, int*send_displ, MPI_Datatype sendtype, double *recv_buf, int*recvcount, int*recv_displ, MPI_Datatype recvtype, int COMM)
{
	int world_rank;
	MPI_Comm_rank(COMM, &world_rank);
	int world_size;
