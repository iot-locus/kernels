...
void get_input(int rank, int start_rank, int n_rank)
{
	MPI_Recv(input_vec, 4, MPI_FLOAT, rank - 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

	//printf("rece from %d in $d\n", rank - 1, rank);

	if(rank != 9)
	{
		MPI_Send(input_vec, 4, MPI_FLOAT, rank + 1, 0, MPI_COMM_WORLD);
		//printf("received the values from the output core ID %d\n", rank);
	}
	/*	
	int x = start_rank, i;
	if(id == start_rank)
